...
public class DictionaryController {
    ...
    @Autowired
    private TushuService tushuService; // 注入TushuService
...
//            return R.error(511,"永远不会进入");
    ...
    public R delete(@RequestBody Integer[] ids) {
        logger.debug("delete: ids={}", Arrays.toString(ids));

        for (Integer id : ids) {
            // 1. 根据字典ID查询对应的字典记录
            DictionaryEntity dictionary = dictionaryService.selectById(id);
            if (dictionary == null) {
                logger.error("字典ID={}不存在", id);
                return R.error("字典ID=" + id + "不存在");
            }

            // 2. 获取字典记录中的code_index（图书类型标识）
            Integer codeIndex = dictionary.getCodeIndex();
            logger.info("字典ID={}对应的图书类型code_index={}", id, codeIndex);

            // 3. 使用code_index删除关联的图书
            boolean isBooksDeleted = tushuService.deleteTushusByTypeId(codeIndex);
            if (!isBooksDeleted) {
                logger.error("删除图书类型失败，字典ID={}, code_index={}", id, codeIndex);
                return R.error("删除图书类型失败，字典ID=" + id);
            }
        }

        // 4. 删除字典记录
        boolean isDictionaryDeleted = dictionaryService.deleteBatchIds(Arrays.asList(ids));
        if (isDictionaryDeleted) {
            return R.ok();
        } else {
            return R.error("删除字典类型失败");
        }
    }
