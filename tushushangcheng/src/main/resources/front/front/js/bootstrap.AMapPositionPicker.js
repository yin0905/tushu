...
(function (factory) {
  ...
  var Position = (function () {
    ...
    Position.validateLngLat = function (lnglatStr) {
      var result =
        /^([+-]?(0?\d{1,2}(\.\d{1,6})?|1[0-7]?\d{1}(\.\d{1,6})?|180\.0{1,6}))[-;,]([-+]?([0-8]?\d{1}(\.\d{1,6})?|90(\.0{1,6})?))$/.exec(
          lnglatStr,
        );
      if (result) {
        return {
          longitude: parseFloat(result[1]),
          latitude: parseFloat(result[5]),
        };
      } else {
        return null;
      }
    };
    Position.LNGLAT_FORMATTER = [
      "{longitude}-{latitude}",
      "{longitude};{latitude}",
      "{longitude},{latitude}",
    ];
  ...
  function buildBootstrapModalHTML() {
    var toolsHtml =
      ...
      "</div>";
    var searchPanelHtml =
      '<div id="idAMapPositionPickerSearchPanel" class="collapse"><input class="form-control input-sm" id="idAMapPositionPickerSearchInput"/><ul id="idAMapPositionPickerSearchResult" class="list-group"></ul></div>';
    var mapPanelHtml =
      '<div id="idAMapPositionPickerFloatContainer" style="position: absolute;z-index: 2;top:5px;right: 5px;">' +
      toolsHtml +
      searchPanelHtml +
      "</div>";
    var modalHtml =
      '<div class="modal fade" id="idAMapPositionPickerModal">' +
  ...
  var PICKER_CONTROLLER = (function () {
    ...
    function showModal() {
      ...
      if (p && p.isValid()) {
        ...
        if (mPicker.opts.center.longitude && mPicker.opts.center.latitude) {
          mapObj.setCenter(
            new AMap.LngLat(
              mPicker.opts.center.longitude,
              mPicker.opts.center.latitude,
            ),
          );
        }
      }
    ...
    function initializeController() {
      ...
      if ($modal == null) {
        ...
        $("ul#idAMapPositionPickerSearchResult").on(
          "click",
          "li[data-poi-index]",
          highlightMarker,
        );
    ...
    function startSearch() {
      ...
      $searchInput.on("keydown", function (e) {
        ...
        if (e.which == "13" && searchKeyword.length > 0) {
          //Enter
              ...
              function (status, result) {
                ...
                if (status == "complete") {
                  for (var i in result.poiList.pois) {
                    ...
                    var li = $(
                      '<li data-poi-index="{i}" class="list-group-item"><small>{name}</small></li>'.format(
                        {
                          i: i,
                          name: poi.name,
                        },
                    ...
                    var mMarker = createMarkerFromLnglat(
                      poi.location.lng,
                      poi.location.lat,
                    );
                  ...
                  $searchPanel.append(
                    '<li class="list-group-item disabled"><small>抱歉，暂无找到符合条件的结果。</small></li>',
                  );
                }
              },
            );
      ...
      this.each(function () {
        ...
        if (!instance) {
          throw new Error(
            'AMapPositionPicker("' +
              options +
              '") method was called on an element that is not using AMapPositionPicker',
          );
        }
