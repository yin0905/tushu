...
(function (domGlobals) {
  ...
  var fromTable = function (table) {
    ...
    return map(rows, function (row) {
      ...
      var cells = map(TableLookup.cells(row), function (cell) {
        var rowspan = has$1(cell, "rowspan")
          ? parseInt(get$1(cell, "rowspan"), 10)
          : 1;
        var colspan = has$1(cell, "colspan")
          ? parseInt(get$1(cell, "colspan"), 10)
          : 1;
  ...
  function DomUniverse() {
    ...
    var isNonEditable = function (element) {
      return (
        isElement(element) && get$1(element, "contenteditable") === "false"
      );
    };
    ...
    return descendant$1(container, firstSelectedSelector).bind(
      function (first) {
        return descendant$1(container, lastSelectedSelector).bind(
          function (last) {
            return DomParent.sharedOne(lookupTable, [first, last]).map(
              function (tbl) {
                return {
  ...
  var elementToData = function (element) {
    var colspan = has$1(element, "colspan")
      ? parseInt(get$1(element, "colspan"), 10)
      : 1;
    var rowspan = has$1(element, "rowspan")
      ? parseInt(get$1(element, "rowspan"), 10)
      : 1;
    return {
      ...
      __assign(
        {
          ...
          cellpadding:
            ...
            Styles$1.getTDTHOverallStyle(editor.dom, elm, "padding"),
  ...
  var open = function (editor) {
    ...
    var cellsData = global$1.map(cells, function (cellElm) {
      return Helpers.extractDataFromCellElement(
        editor,
        cellElm,
  ...
  var open$1 = function (editor) {
    ...
    var rowsData = global$1.map(rows, function (rowElm) {
      return Helpers.extractDataFromRowElement(
        editor,
        rowElm,
  ...
  var open$2 = function (editor, insertNewTable) {
    var dom = editor.dom;
    var tableElm;
    var data = Helpers.extractDataFromSettings(
      editor,
  ...
  var calculatePercentageWidth = function (element, parent) {
    return (
      (getPixelWidth$1(element.dom()) / getPixelWidth$1(parent.dom())) * 100 +
      "%"
    );
  };
  ...
  var enforcePixels = function (table) {
    set$1(
      ...
      "width",
      getPixelWidth$1(table).toString() + "px",
    );
  };
  ...
  var tryAt = function (bridge, direction, box) {
    if (
      platform$2.browser.isChrome() ||
      platform$2.browser.isSafari() ||
      platform$2.browser.isFirefox() ||
      platform$2.browser.isEdge()
    ) {
